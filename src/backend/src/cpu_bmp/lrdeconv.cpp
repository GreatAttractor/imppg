...
{
    IMPPG_ASSERT(input.GetPixelFormat() == PixelFormat::PIX_MONO32F || input.GetPixelFormat() == PixelFormat::PIX_RGB32F);
    IMPPG_ASSERT(input.GetPixelFormat() == output.GetPixelFormat());

    const std::size_t numChannels = NumChannels[static_cast<std::size_t>(input.GetPixelFormat())];
    const unsigned width = input.GetWidth();
    const unsigned height = input.GetHeight();

    auto prev = std::unique_ptr<float[]>(new float[width * height * numChannels]);
    auto next = std::unique_ptr<float[]>(new float[width * height * numChannels]);

    auto inputConvolvedDivT = std::unique_ptr<float[]>(new float[height * width * numChannels]); // a transposed array
    auto estimateConvolvedT = std::unique_ptr<float[]>(new float[height * width * numChannels]); // a transposed array
    auto conv2 = std::unique_ptr<float[]>(new float[width * height]);

    auto inputT = std::unique_ptr<float[]>(new float[height * width]); // a transposed array

    if (1 == numChannels)
    {
        Transpose<float, 1>(
            input.GetRowAs<const float>(0),
            inputT.get(),
            width,
            height,
            input.GetBytesPerRow(),
            input.GetHeight() * numChannels * sizeof(float),
    ...
    auto tempBuf1 = std::unique_ptr<float[]>(new float[width * height * numChannels]);
    auto tempBuf2 = std::unique_ptr<float[]>(new float[width * height * numChannels]);

    const int kernelRadius = static_cast<int>(ceil(sigma * 3.0f));
    ...
    {
        memcpy(prev.get() + i * width, input.GetRow(i), width * numChannels * sizeof(float));
    }
    ...
    for (int i = 0; i < numIters; i++)
    {
        ...
        for (unsigned j = 0; j < height * width * numChannels; j++)
        {
            inputConvolvedDivT[j] = inputT[j] / (estimateConvolvedT[j] + 1.0e-8f); // add a small epsilon to prevent division by 0 and propagation of Infs across output pixels
        }
        ...
        #pragma omp parallel for
        for (unsigned j = 0; j < width * height * numChannels; j++)
        {
    ...
    for (unsigned i = 0; i < height; i++)
    {
        memcpy(output.GetRow(i), next.get() + i * width, width * numChannels * sizeof(float));
    }
}
...
)
{
    ...
    ConvolveSeparable(
        ...
        sigma,
        NumChannels[static_cast<std::size_t>(input.GetPixelFormat())]
    );
