...
{
    return m_OutputDirCtrl->GetPath();
}

OutputFormat c_BatchParamsDialog::GetOutputFormat()
{
    return static_cast<OutputFormat>(m_OutputFormatsCtrl->GetCurrentSelection());
}

wxString c_BatchParamsDialog::GetSettingsFileName()
{
    return m_SettingsFileCtrl->GetPath();
}


void c_BatchParamsDialog::OnSettingsFileChanged(wxFileDirPickerEvent& event)
{
    m_SettingsFileCtrl->SetLabel(event.GetPath());
}
...
{
    Configuration::BatchLoadSettingsPath = wxFileName(m_SettingsFileCtrl->GetPath()).GetPath();
    Configuration::BatchDialogPosSize = wxRect(GetPosition(), GetSize());
    Configuration::BatchOutputPath = m_OutputDirCtrl->GetPath();
    Configuration::BatchOutputFormat = static_cast<OutputFormat>(m_OutputFormatsCtrl->GetSelection());
}
...
void c_BatchParamsDialog::OnCommandEvent(wxCommandEvent& event)
{
    ...
    {
    case ID_Start:
        if (!wxFileName::DirExists(m_OutputDirCtrl->GetPath()))
        ...
        else if (!wxFileName::FileExists(m_SettingsFileCtrl->GetPath()))
...
void c_BatchParamsDialog::DoInitControls()
{
    ...
    szTop->Add(m_FileList = new wxListBox(GetContainer(), ID_FileList, wxDefaultPosition, wxDefaultSize, wxArrayString(), wxLB_EXTENDED),
        1, wxALIGN_CENTER | wxGROW | wxALL, BORDER);
    ...
    szProcSettings->Add(m_SettingsFileCtrl = new wxFilePickerCtrl(GetContainer(), ID_SettingsFilePicker, "", _("Choose the file with processing settings"),
        _("XML files (*.xml)") + "|*.xml", wxDefaultPosition, wxDefaultSize, wxFD_OPEN | wxFD_FILE_MUST_EXIST),
        0, wxALIGN_CENTER_VERTICAL | wxALL, BORDER);
    m_SettingsFileCtrl->SetInitialDirectory(Configuration::BatchLoadSettingsPath);
    ...
    szOutputDir->Add(m_OutputDirCtrl = new wxDirPickerCtrl(GetContainer(), ID_OutputDir, Configuration::BatchOutputPath, _("Select output folder")),
    ...
    for (int i = 0; i < static_cast<int>(OutputFormat::LAST); i++)
        formatStr.Add(GetOutputFormatDescription(static_cast<OutputFormat>(i)));
    m_OutputFormatsCtrl = new wxChoice(GetContainer(), ID_OutputFormat, wxDefaultPosition, wxDefaultSize, formatStr);
    m_OutputFormatsCtrl->SetSelection(static_cast<int>(static_cast<OutputFormat>(Configuration::BatchOutputFormat)));
    szOutFmt->Add(m_OutputFormatsCtrl, 0, wxALIGN_CENTER_VERTICAL | wxALL, BORDER);
