...
: c_WorkerThread(parent, guard, instancePtr, taskId, input, output, threadId),
  m_Adaptive(adaptive), m_Sigma(sigma), m_AmountMin(amountMin), m_AmountMax(amountMax), m_Width(width), m_Threshold(threshold), m_RawInput(rawInput)
{
...
void c_UnsharpMaskingThread::DoWork()
{
    ...
    boost::scoped_array<float> gaussianImg(new float[width * height]);
    ...
    */
        ...
        boost::scoped_array<float> imgL(new float[width * height]);
        ConvolveSeparable(
