...
File description:
    Fragment shader: horizontal Gaussian convolution (blur).
*/
...
void main()
{
    if (IsMono)
    {
        ...
        {
            outputValue += GaussianKernel[abs(i)] * texture(Image, TexCoord + vec2(float(i), 0)).r;
        }
    ...
    else
    {
        ...
        for (int i = -(KernelRadius - 1); i < KernelRadius; i++)
        {
            outputValue += GaussianKernel[abs(i)] * texture(Image, TexCoord + vec2(float(i), 0)).rgb;
        }
