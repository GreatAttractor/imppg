    ...
    )
{
    ...
    boost::scoped_ptr<c_Image> srcImg;
    boost::scoped_ptr<c_Image> outpImg;
...
#endif
    {
        ...
        IImageBuffer *fibuf = new c_FreeImageBuffer(fiSrcBmp);

        srcImg.reset(new c_Image(fibuf->GetWidth(), fibuf->GetHeight(), fibuf->GetPixelFormat(), fibuf));
        ...
        fibuf = new c_FreeImageBuffer(fiOutpBmp);
        outpImg.reset(new c_Image(fibuf->GetWidth(), fibuf->GetHeight(), fibuf->GetPixelFormat(), fibuf));
    ...
    )
{
    ...
    if (intrWidth >= STBL_AREA_SIZE && intrHeight >= STBL_AREA_SIZE)
    {
        ...
        boost::scoped_ptr<c_Image> firstImg(LoadImageFileAsMono32f(m_Parameters.inputFiles[0].ToStdString(), wxFileName(m_Parameters.inputFiles[0]).GetExt().Lower().ToStdString()));
        if (!firstImg.get())
        {
        ...
        boost::scoped_ptr<c_Image> wndFunc(CalcWindowFunction(STBL_AREA_SIZE, STBL_AREA_SIZE));
        ...
        boost::scoped_ptr<c_Image> prevArea(new c_Image(STBL_AREA_SIZE, STBL_AREA_SIZE, PIX_MONO32F));
        ...
        for (size_t i = 1; i < m_Parameters.inputFiles.Count(); i++)
        {
            ...
            boost::scoped_ptr<c_Image> currImg(LoadImageFileAsMono32f(m_Parameters.inputFiles[i].ToStdString(), wxFileName(m_Parameters.inputFiles[i]).GetExt().Lower().ToStdString()));
            if (!currImg.get())
            {
            ...
            boost::scoped_ptr<c_Image> currArea(new c_Image(STBL_AREA_SIZE, STBL_AREA_SIZE, PIX_MONO32F));
...
void CountNeighborsAboveThreshold(
    ...
    int &numAbove, ///< Receives the number of pixels above threshold
    int &numTotal ///< Receives the number of all neighbor pixels used
)
{
...
)
{
    ...
    for (size_t i = 0; i < m_Parameters.inputFiles.Count(); i++)
    {
        ...
        boost::scoped_ptr<c_Image> img(LoadImageFileAsMono8(m_Parameters.inputFiles[i].ToStdString(), wxFileName(m_Parameters.inputFiles[i]).GetExt().Lower().ToStdString()));
        if (!img.get())
        {
        ...
        int numPointsExceedingMaxFraction = 0;
        for (size_t j = 0; j < (int)limbPoints[i].size(); j++)
        {
            int numTotal, numAbove;
