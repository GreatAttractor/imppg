...
void GetRayPoints(Point_t origin, Point_t dir, c_Image &img, Ray_t &result)
{
    ...
    if (dir.x == 0)
    {
        ...
        for (int y = origin.y; y >= 0 && y < img.GetHeight(); y += delta)
    ...
    else if (dir.y == 0)
    {
        ...
        for (int x = origin.x; x >=0 && x < img.GetWidth(); x += delta) 
...
)
{
    ...
    while (iHigh - iLow > 1)
    {
        float varSum = GetSumSqrDiffsFromHistogram(histogram, 0, currDivPos)
                + GetSumSqrDiffsFromHistogram(histogram, currDivPos, 255);
    ...
    int totalDisc = 0, discCount = 0,
             totalBkgrnd = 0, bkgrndCount = 0;
    for (int i = 0; i < currDivPos; i++)
    {
        totalBkgrnd += (int)i*histogram[i];
        bkgrndCount += histogram[i];
    }
    for (int i = currDivPos; i < 256; i++)
    {
        totalDisc += (int)i*histogram[i];
