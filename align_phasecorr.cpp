...
/// Class to automatically delete an array allocated with 'operator new[]'
/* Used instead of 'std::unique_ptr' or 'boost::interprocess::unique_ptr'/'boost::move::unique_ptr' (with a custom deleter) for simplicity
  (avoiding wxWidgets problems with -std=c++11 on MinGW, 'swap' resolution ambiguity in MSVC, Boost 1.57/1.55 portability).
  Eventually to be replaced by 'std::unique_ptr'. **/
template<typename T>
class c_InPlaceArrayDeleter
{
    T *ptr;
public:
    c_InPlaceArrayDeleter(void *ptr): ptr((T *)ptr) { }
    ~c_InPlaceArrayDeleter() { operator delete[]((void *)ptr); }
    /// Both objects need to have valid pointers!
    void swap(c_InPlaceArrayDeleter &a)
    {
        T *temp = this->ptr;
        this->ptr = a.ptr;
        a.ptr = temp;
...
{
    c_Image *result = new c_Image(wndWidth, wndHeight, PIX_MONO32F);
 ...
 */
    ...
    c_InPlaceArrayDeleter<std::complex<float> > cps(operator new[](Nwidth*Nheight * sizeof(std::complex<float>)));
    // Cross-correlation
    c_InPlaceArrayDeleter<std::complex<float> > cc(operator new[](Nwidth*Nheight * sizeof(std::complex<float>)));

    CalcCrossPowerSpectrum2D(img1FFT, img2FFT, cps, Nwidth*Nheight);
    CalcFFTinv2D(cps, Nheight, Nwidth, cc);
    ...
    for (unsigned y = 0; y < Nheight; y++)
        ...
        {
            float currval = cc[x + y*Nwidth].real();
            if (currval > maxval)
            {
                maxval = currval;
                maxx = x;
    ...
    if (subpixelAccuracy)
    {
        ...
        float ccXhi = cc[CLAMPW(maxx+1) + maxy*Nwidth].real(),
              ccXlo = cc[CLAMPW(maxx-1) + maxy*Nwidth].real(),
              ccYhi = cc[maxx + CLAMPH(maxy+1)*Nwidth].real(),
              ccYlo = cc[maxx + CLAMPH(maxy-1)*Nwidth].real(),
              ccPeak = cc[maxx + maxy*Nwidth].real();
...
)
{
    ...
    c_InPlaceArrayDeleter<std::complex<float> > fft1(operator new[](width * height * sizeof(std::complex<float>)));
    c_InPlaceArrayDeleter<std::complex<float> > fft2(operator new[](width * height * sizeof(std::complex<float>)));

    CalcFFT2D((float *)img1.GetRow(0), height, width, img1.GetBuffer().GetBytesPerRow(), fft1);
    CalcFFT2D((float *)img2.GetRow(0), height, width, img2.GetBuffer().GetBytesPerRow(), fft2);

    return DetermineImageTranslation(width, height, fft1, fft2, true);
}
...
)
{
    ...
    boost::scoped_ptr<c_Image> windowFunc(CalcWindowFunction(Nwidth, Nheight));
    ...
    boost::scoped_ptr<c_Image> prevImg(new c_Image(Nwidth, Nheight, PIX_MONO32F)); // previous image in the sequence (padded to Nwidth*Nheight pixels and with window func. applied)
    boost::scoped_ptr<c_Image> currImg(new c_Image(Nwidth, Nheight, PIX_MONO32F)); // current image in the sequence (padded to Nwidth*Nheight pixels and with window func. applied)
    ...
    c_InPlaceArrayDeleter<std::complex<float> > currFFT(operator new[](Nwidth * Nheight * sizeof(std::complex<float>)));
    ...
    CalcFFT2D((float *)prevImg->GetRow(0), prevImg->GetHeight(), prevImg->GetWidth(), prevImg->GetBuffer().GetBytesPerRow(), prevFFT);
    Log::Print("done.");
    ...
    for (unsigned i = 1; i < inputFiles.size(); i++)
    {
        ...
        Log::Print("Calculating FFT... ");
        CalcFFT2D((float *)currImg->GetRow(0), currImg->GetHeight(), currImg->GetWidth(), currImg->GetBuffer().GetBytesPerRow(), currFFT);
        Log::Print("done.\n");

        FloatPoint_t T = DetermineImageTranslation(Nwidth, Nheight, prevFFT, currFFT, subpixelAlignment);
