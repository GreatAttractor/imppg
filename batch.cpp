...
class c_BatchDialog: public wxDialog
{
    ...
    int m_CurrentFile; ///< Index of the currently processed file
...
{
    if (m_Img == 0) // Image is not yet created, i.e. 'request' is the first processing step specified in the settings file
    {
        wxFileName path = wxFileName(m_FileNames[m_CurrentFile]);
        wxString ext = path.GetExt().Lower();
        std::string errorMsg;
        c_Image *newImg = LoadImageFileAsMono32f(path.GetFullPath().ToStdString(), path.GetExt().ToStdString(), &errorMsg);
        if (newImg == 0)
        {
