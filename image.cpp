...
public:
    ...
    virtual int GetBytesPerRow() { return width * bytesPerPixel; }
    virtual int GetBytesPerPixel() { return bytesPerPixel; }
    virtual void *GetRow(int row) { return (uint8_t *)buffer + row * width * bytesPerPixel; }
    virtual PixelFormat_t GetPixelFormat() { return pixFmt; }
    virtual IImageBuffer *GetCopy()
    {
        c_SimpleBuffer *copy = new c_SimpleBuffer(width, height, pixFmt);
...
c_Image::c_Image(): width(0), height(0), pixels(0) { }

/// Allocates memory for pixel data
c_Image::c_Image(unsigned width, unsigned height, PixelFormat_t pixFmt,
    IImageBuffer *buffer ///< If null, c_Image allocates its own; if not null, c_Image will use it and free it in destructor):
): width(width), height(height)
{
    assert(pixFmt > PIX_UNCHANGED && pixFmt < PIX_NUM_FORMATS);
    assert(width > 0 && height > 0);
    assert(buffer == 0 || buffer->GetPixelFormat() == pixFmt);

    if (buffer)
        pixels = buffer;
    else
    {
        pixels = new c_SimpleBuffer(width, height, pixFmt);
    }
...
c_Image::c_Image(const c_Image &img):
    ...
    height(img.height)
{
    memcpy(palette, img.palette, sizeof(palette));
...
c_Image *c_Image::ConvertPixelFormat(c_Image &srcImg, PixelFormat_t destPixFmt, int x0, int y0, int width, int height)
{
    ...
    assert(x0 >= 0 && x0 < srcImg.GetWidth());
    assert(y0 >= 0 && y0 < srcImg.GetHeight());
...
void c_Image::Copy(c_Image &src, c_Image &dest, int srcX, int srcY, int width, int height, int destX, int destY)
{
...
)
{
    ...
    int destXend = std::min(xOfsInt + srcXmax, destImg.GetWidth()-1);
    int destYend = std::min(yOfsInt + srcYmax, destImg.GetHeight()-1);
    ...
    if (clearToZero)
    {
        ...
        for (int y = destYend + 1; y < destImg.GetHeight(); y++)
            memset(destImg.GetRow(y), 0, destImg.GetWidth() * bytesPP);
        ...
        case OUTF_FITS_16:
            {
                ...
                for (int row = 0; row < img16->GetHeight(); row++)
        ...
        case OUTF_FITS_8:
            {
                ...
                for (int row = 0; row < img8->GetHeight(); row++)
