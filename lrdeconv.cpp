...
)
{
    ...
    boost::scoped_array<float> outputT(new float[input.height()*input.width()]); // transposed output
    boost::scoped_array<float> temp1(new float[input.width()*input.height()]);
    boost::scoped_array<float> temp2(new float[input.width()*input.height()]);

    if (kernelRadius < YOUNG_VAN_VLIET_MIN_KERNEL_RADIUS)
    {
        boost::scoped_array<float> kernel(new float[2 * kernelRadius - 1]);
...
)
{
    ...
    for (int i = 0; i < numIters; i++)
    {
        ...
        #pragma omp parallel for
        for (int j = 0; j < input.GetHeight() * input.GetWidth(); j++)
        ...
        #pragma omp parallel for
        for (int j = 0; j < input.GetWidth() * input.GetHeight(); j++)
...
)
{
        ...
        for (x = 0; x < input.GetWidth(); x++)
        {
            ...
            if (valXYbelowThreshold)
                for (j = -1; j <= 1; j++)
                    for (i = -1; i <= 1; i++)
                        if (i != 0 && j != 0 &&
                            x + i >= 0 && x + i < input.GetWidth() &&
                            y + j >= 0 && y + j < input.GetHeight())
                        {
    ...
    {
        decodeXY(*it, x, y);
        for (i = -(influenceDist - 1); i <= influenceDist - 1; i++)
            for (j = -(influenceDist - 1); j <= influenceDist - 1; j++)
    ...
    for (std::set<unsigned>::iterator it = influencedPixels.begin(); it != influencedPixels.end(); it++)
    {
        ...
        for (i = -(blurRadius - 1); i <= blurRadius - 1; i++)
            for (j = -(blurRadius - 1); j <= blurRadius - 1; j++)
            {
                int srcX = x + i;
                int srcY = y + j;
